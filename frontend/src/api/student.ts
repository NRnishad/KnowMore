...
export const verifyPaymentApi = async (sessionId: string, orderId: string) => {
  try {
    const response = await api.post(`${API_URL}/student/order/verify`, { sessionId, orderId }, { withCredentials: true });
    return response.data;
  } catch (error: any) {
